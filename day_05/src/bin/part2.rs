...
fn part2(input: &str) -> i64 {
    ...
    // Updated Seed numbers
    let mut true_seed_nums: Vec<i64> = vec![];
    let mut last_seed_num = 0;
    for (e, seed_num) in seed_nums.iter().enumerate() {
        if e % 2 == 0 {
            last_seed_num = seed_num.clone();
            println!("{:?}", seed_num);
        } else {
            for add_seed_num in last_seed_num..last_seed_num + seed_num {
                true_seed_nums.push(add_seed_num);
            }
        }
    }

    // Painfully slow brute force solving - could potentially flatten maps into an input = output
    // map for a much more efficient solve
    let mut lowest_location_numbers: i64 = 0;
    for (i, true_seed_num) in true_seed_nums.iter().enumerate() {
        let mut out_num = true_seed_num.clone();
        ...
        if out_num < lowest_location_numbers || lowest_location_numbers == 0 {
            lowest_location_numbers = out_num;
        }
    }
    lowest_location_numbers
}
